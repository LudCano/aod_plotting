...
data_path = Path('descarga_frp')
# Collect all of the subsetted ABI FDC data files in the "abi_files" directory
file_list = sorted(data_path.glob('*FDC*.nc'))

ds = xr.open_dataset(file_list[0])
latitude_array, longitude_array = calculate_abi_lat_lon(ds)


# Set up a loop to plot data from multiple ABI files
stacked = []
for file in file_list[:]:
    # Open file using xarray (automatically closes file when done)
    with xr.open_dataset(file, engine='netcdf4') as ds:

        # Convert "Mask" DataArray to correct data type
        Mask = ds.Mask.astype('int16')

        # Process latitude, longitude arrays using "Mask" variable
        # Retain only "good" and "saturated" fire pixels
        # np.isnan() returns boolean array with NaN=="True"
        # np.logical_not() reverses the boolean array
        fire_mask = np.logical_not(np.isnan(ds.Power.where((Mask == 10) | (Mask == 30) | (Mask == 11) | (Mask == 31))))
        # Apply mask of reversed boolean array to latitude/longitude arrays
        # Returns 1D arrays
        fire_latitude = latitude_array[fire_mask]
        fire_longitude = longitude_array[fire_mask]

        fire_coords = np.array([fire_latitude, fire_longitude])
        stacked.append(fire_coords)

stacked = np.hstack(stacked)
uniques = np.unique(stacked, axis = 1)
fires_lats = uniques[0,:]
fires_lons = uniques[1,:]
...
sc = ax.scatter(fire_longitude, fire_latitude, s = 5, marker = 'x', c = 'k', transform = ccrs.PlateCarree())
# Add colorbar
...
fig.savefig(f'composites/composite{forfname}_frp.png', dpi = 500)
if os.path.exists('current_plots/composite_frp.png'):
